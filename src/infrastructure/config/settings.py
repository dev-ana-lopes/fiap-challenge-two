    ...
    def validate_configuration(self) -> "Settings":
        ...
        if self.ENVIRONMENT in {"staging", "production"}:
            if len(self.JWT_SECRET) < 32 or _looks_like_placeholder(self.JWT_SECRET):
                raise ValueError(
                    "JWT_SECRET must be a strong non-placeholder secret in "
                    "staging/production"
                )
            ...
            if len(self.approval_token_secret) < 32 or _looks_like_placeholder(
                ...
                raise ValueError(
                    "APPROVAL_TOKEN_SECRET must be a strong non-placeholder secret in "
                    "staging/production"
                )

            if self.approval_token_secret == self.JWT_SECRET:
                raise ValueError(
                    "APPROVAL_TOKEN_SECRET must be different from JWT_SECRET in "
                    "staging/production"
                )
